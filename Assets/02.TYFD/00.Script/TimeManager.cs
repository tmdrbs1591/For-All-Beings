...
public class TimeManager : MonoBehaviour
{
    ...
    private void SetTimerText()
    {
        if (currentTime >= 60)
        {
            ...
            {
                timerText.text =  Mathf.FloorToInt(currentTime / 60).ToString() + " : 0" + Mathf.FloorToInt(currentTime % 60);
            }
            else
            {
                timerText.text = Mathf.FloorToInt(currentTime / 60).ToString() + " : " + Mathf.FloorToInt(currentTime % 60);
            }
        }
        else if (currentTime < 60)
        {
            if (currentTime % 60 < 10)
            {
                timerText.text = " 0 : 0" + Mathf.FloorToInt(currentTime % 60);
            }
            else
            {
                timerText.text = " 0 : " + Mathf.FloorToInt(currentTime % 60);
            }
        }
    }
