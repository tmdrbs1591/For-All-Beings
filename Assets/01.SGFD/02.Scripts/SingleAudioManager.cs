...
public class SingleAudioManager : MonoBehaviour
{
    ...
    {
        // 오디오 오브젝트 생성
        SingleAudioObject audObject = GameObject.Instantiate(audioObject, new Vector3(position.x, position.y, -5), Quaternion.identity).GetComponent<SingleAudioObject>();

        // 오디오 오브젝트를 따라다닐 대상 설정
        audObject.follow = follower;

        // 오디오 클립 설정
        audObject.clip = clips[index];

        // 피치 설정
        audObject.pitch = pitch;
