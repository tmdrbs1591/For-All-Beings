    ...
    void Attack()
    {
        if (attacklCurTime <= 0)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (randomIndex == 0)
                {
                    SingleAudioManager.instance.PlaySound(transform.position, 9, UnityEngine.Random.Range(1f, 1f), 0.3f);
                }
                else if (randomIndex == 1)
                {
                    SingleAudioManager.instance.PlaySound(transform.position, 10, UnityEngine.Random.Range(1f, 1f), 0.3f);
                }
                else
                {
